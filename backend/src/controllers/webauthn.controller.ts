...
const buildSessionResponse = async (user: UserWithBranch, req: Request) => ({
  token: generateToken({
    userId: user.id,
    email: user.email,
    role: user.role,
    ...
    jti: await openAdminSession(user.id, user.branchId, { ip: clientIp(req), device: getRequestDeviceId(req) }),
  }),
  user: {
    id: user.id,
    email: user.email,
...
export const webauthnRegisterVerify = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res.status(200).json({ message: "Dispositivo registrado. Acceso autorizado.", ...(await buildSessionResponse(user, req)) });
  } catch (error: any) {
...
export const webauthnLoginVerify = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res.status(200).json({ message: "Acceso autorizado.", ...(await buildSessionResponse(user, req)) });
  } catch (error: any) {
