...
const FacturacionGlobalView: React.FC<ViewProps> = ({ branchId, refreshToken }) => {
  ...
  const loadEligibleTickets = useCallback(async () => {
    ...
    try {
      ...
      // Filtramos las que no estén facturadas en el frontend (cfdiUuid === null/empty)
      const sales = res.data.sales || [];
      const nonInvoicedSales = sales.filter((s: any) => !s.cfdiUuid);
      setTickets(nonInvoicedSales);
    } catch (err: any) {
  ...
  const hasAssociatedCustomers = tickets.some(
    (t: any) => t.customer && t.customer.toLowerCase() !== "público general"
  );
          ...
          {hasAssociatedCustomers && (
            <div style={warningAlert}>
              ...
              <div>
                ...
                <p style={{ fontSize: 13, marginTop: 4 }}>
                  Los tickets con clientes asociados no podrán facturar si se incluyen en la factura global.
                </p>
              </div>
            </div>
          )}
