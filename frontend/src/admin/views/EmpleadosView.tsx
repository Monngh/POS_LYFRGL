...
const EmpleadosView: React.FC<ViewProps> = ({ branchId, refreshToken }) => {
                  ...
                  {checkPasswordRequirements(form.password).map((req) => (
                    <div
                      ...
                      style={{
                        ...
                        alignItems: "center",
                        gap: 6,
                        color: req.passed ? "#16a34a" : "var(--text-muted)",
                        marginBottom: 2,
                      }}
                    >
  ...
  const renderMobileView = () => (
    <div style={{ overflowY: "auto", maxHeight: "62vh", padding: "8px 4px" }}>
      ...
      {!loading && error && <div style={{ textAlign: "center", padding: "32px 16px", color: "#b91c1c", fontSize: 13, fontWeight: 500 }}>{error}</div>}
      ...
      >
        <div style={{ padding: "10px 0" }}>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
            ...
            <button style={{ ...ui.primaryBtn, backgroundColor: "#dc2626", borderColor: "#dc2626" }} onClick={closeForm}>Descartar</button>
          </div>
        </div>
      </ActionModal>
...
const Mini: React.FC<{
  ...
  const isMobile = useMediaQuery("(max-width: 1024px)");
  const accentStyles = {
    blue: { background: "#eff6ff", borderLeft: "4px solid #60a5fa", valueColor: "#2563eb" },
    green: { background: "#f0fdf4", borderLeft: "4px solid #4ade80", valueColor: "#16a34a" },
    red: { background: "#fef2f2", borderLeft: "4px solid #f87171", valueColor: "#dc2626" },
  };
